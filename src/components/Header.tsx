      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          <Link href="/" className="flex items-center space-x-3 select-none">
            <span className="text-international-orange-500 font-black text-2xl tracking-tight hover:scale-105 transition-transform duration-200">
              Brewcode
            </span>
          </Link>
        ...
        {/* Mobile Navigation */}
        {isMenuOpen && (
          <div className="md:hidden bg-white dark:bg-gray-900 border-t-2 border-international-orange-500 shadow-[0_4px_0_0_rgba(0,0,0,0.1)]">
            <nav className="px-2 pt-2 pb-3 space-y-1">
              {navItems.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  className="block px-3 py-3 text-gray-800 dark:text-gray-200 hover:text-international-orange-500 hover:bg-international-orange-500/10 font-bold text-lg rounded transition-all duration-200"
